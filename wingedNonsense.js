...
window.wingedNonsense.alterSnakeCode = function(code) {
  ...
  code = assertReplace(
    code,
    /Oc\.pos\.x===kd\.x\?zTD\(Oc,[^)]*\):Oc\.we\.x=Oc\.pos\.x<kd\.x\?\.5:-\.5;Oc\.pos\.y===kd\.y\?zTD\(Oc,[^)]*\):Oc\.we\.y=Oc\.pos\.y<kd\.y\?\.5:-\.5;/,
    "Oc.pos.x===kd.x?zTD(Oc,!0,!1):Oc.we.x=Oc.pos.x<kd.x?-.8:.8;Oc.pos.y===kd.y?zTD(Oc,!1,!0):Oc.we.y=Oc.pos.y<kd.y?-.8:.8;"
  );
  // adds randomization in the velocity vector of the apple that is supposed to be 0
  code = assertReplace(
    code,
    /b&&a\.we\.x!==0&&\(a\.Lzb\.x=a\.we\.x,a\.we\.x=0\);c&&a\.we\.y!==0&&\(a\.Lzb\.y=a\.we\.y,a\.we\.y=0\)/,
    "b&&a.we.x!==0&&(a.Lzb.x=a.we.x,a.we.x=0,a.we.y+=(Math.random()*.6-.3));c&&a.we.y!==0&&(a.Lzb.y=a.we.y,a.we.y=0,a.we.x+=(Math.random()*.6-.3))"
  );

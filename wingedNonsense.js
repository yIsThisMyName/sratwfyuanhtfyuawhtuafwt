...
window.wingedNonsense.alterSnakeCode = function(code) {
  ...
  code = assertReplace(
    ...
    "for (const a of this.oa){if(!Object.hasOwn(a, \"wingedTimer\"))a.wingedTimer=0;a.wingedTimer++;if(a.wingedTimer%10==0){a.we.x=((Math.floor(Math.random()*3)-1)/2);a.we.y=((Math.floor(Math.random()*3)-1)/2);console.log(\"it worksss\");}a.Oo || (a.yL = (a.yL + 1) % 6,O6(this.settings) && (a.pL.x && (a.pos.x += a.we.x),a.pL.y && (a.pos.y += a.we.y),P6(this.settings, 4) && (a.pos.x < 0 ? a.pos.x += this.Aa.Aa.width : a.pos.x >= this.Aa.Aa.width && (a.pos.x -= this.Aa.Aa.width),a.pos.y < 0 ? a.pos.y += this.Aa.Aa.height : a.pos.y >= this.Aa.Aa.height && (a.pos.y -= this.Aa.Aa.height))))}"
  );
  return code;
}
